...
def create_patient():
    if request.method == "POST":
        try:
            ...
            flash(f"Patient {patient.full_name} created successfully with ID: {patient.id}", "success")
...
def diagnosis_records():
    predictions = Prediction.query.filter_by(doctor_id=current_user.id)\
...
def predict_endometriosis():
    try:
        ...
        diagnosis = (
            ...
            if prob_endo >= 50 else
            "Endometriosis Not Detected"
        )
...
def save_diagnosis():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "message": f"Failed to save diagnosis: {str(e)}"
        }), 500
...
def get_diagnosis_records():
    ...
    try:
        ...
        records = DiagnosisRecord.query.options(
            joinedload(DiagnosisRecord.patient),
            joinedload(DiagnosisRecord.doctor)
        ).filter_by(doctor_id=current_user.id).order_by(DiagnosisRecord.created_at.desc()).all()
...
def get_diagnosis(diagnosis_id):
    ...
    try:
        ...
        diagnosis = DiagnosisRecord.query.options(
            joinedload(DiagnosisRecord.patient),
            joinedload(DiagnosisRecord.doctor)
        ).filter_by(
            id=diagnosis_id, 
...
def get_report_data():
    ...
    try:
        ...
        avg_confidence = sum(r.confidence_score for r in records) / total_diagnoses if total_diagnoses > 0 else 0
        ...
        return jsonify({
            ...
            "statistics": {
                ...
                "positive_percentage": (positive_count / total_diagnoses * 100) if total_diagnoses > 0 else 0,
                "negative_percentage": (negative_count / total_diagnoses * 100) if total_diagnoses > 0 else 0,
...
def export_csv():
    ...
    try:
        ...
        # Write headers
        writer.writerow([
            'Patient Name', 'Patient ID', 'Diagnosis', 'Confidence (%)', 
            'Positive Prob (%)', 'Negative Prob (%)', 'Status', 'Created Date'
        ])
        ...
        response.headers["Content-Disposition"] = f"attachment; filename=diagnosis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def export_excel():
    ...
    try:
        ...
        # Define styles
        header_fill = PatternFill(start_color="2864BD", end_color="2864BD", fill_type="solid")
        ...
        # Write headers (row 4)
        headers = ['Patient Name', 'Patient ID', 'Diagnosis', 'Confidence (%)', 
                   'Positive Prob (%)', 'Negative Prob (%)', 'Status', 'Created Date']
        ...
        for row_idx, record in enumerate(records, 5):
            ws.cell(row=row_idx, column=1).value = record.patient.full_name if record.patient else ''
        ...
        # Create response
        response = make_response(output.getvalue())
        response.headers["Content-Disposition"] = f"attachment; filename=diagnosis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
def export_pdf():
    ...
    try:
        ...
        stats_data = [
            ...
            [str(total_diagnoses), str(positive_count), str(negative_count), 
             f"{(positive_count/total_diagnoses*100):.1f}%" if total_diagnoses > 0 else "0%"]
        ]
        ...
        table_data = [['Patient', 'Patient ID', 'Diagnosis', 'Confidence', 'Status', 'Date']]
        ...
        table = Table(table_data, colWidths=[1.2*inch, 1*inch, 1.3*inch, 1*inch, 1*inch, 1.2*inch])
        ...
        # Create response
        response = make_response(output.getvalue())
        response.headers["Content-Disposition"] = f"attachment; filename=diagnosis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
